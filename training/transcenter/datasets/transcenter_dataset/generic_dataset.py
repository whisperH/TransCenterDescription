...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # ==================
        # see the result of heatmap
        # for img(480,480,3)->expression: cv2.resize(img, (720,720))
        # for ret['hm](1, 720,720)->expression: ret['hm'].transpose(1,2,0)
        # for heatmap + image ->expression: np.abs(ret['hm'].transpose(1,2,0)-1)+ ret['image'].transpose(1,2,0)
        # ==================
        return ret
    ...
    def _add_instance(
        ...
        # 获取中心点的索引，在后面output 输出的时候，根据索引取featuremap对应的点，再进行loss回归
